...
public class Flipkart_AddToCart {
	...
	public static void main(String[] args) throws Exception {
		...
		String validate = ele.isDisplayed()?"Product ListPage is Displayed\n":"Product ListPage is Displayed\n"; 
		System.out.println(validate);
		String parentId= driver.getWindowHandle();

		List<WebElement> allPhones = driver.findElements(By.xpath("//div[contains(text(),'Apple iPhone 16')]"));
		...
		while(itr.hasNext()) {
			...
			for(String singleWin:windowHandles) {
					...
					try {
						...
						WebElement cartButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//li[@class='col col-2-12 ']")));
						cartButton.click();
					}
					...
					try {
						Thread.sleep(100);
						WebElement outOfStock = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[text()='Product is now out of stock.']")));
						if(outOfStock.isDisplayed()) {
							try {
								//Validating Cart
								WebElement cart = driver.findElement(By.xpath("//div[@class='ZuSA--']"));
								if(cart.isDisplayed()) {
